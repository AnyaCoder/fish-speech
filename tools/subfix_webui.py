        ...
        output.append(
            {
                ...
                g_json_key_lang_text: d[g_json_key_lang_text],
...
def b_change_index(index, batch):
    ...
    for i , _ in enumerate(datas):
        output.append(
            ...
            {
                "__type__":"update",
                "label":f"Language {i+index}",
                "value":_[g_json_key_lang_text]
            }
        )
    for _ in range(g_batch - len(datas)):
        output.append(
...
def b_submit_change(*text_list):
    global g_data_json
    change = False
    lang_start_pos = len(text_list) // 2
    for i, new_text in enumerate(text_list):
        if i >= lang_start_pos: break
        if g_index + i <= g_max_json_index:
            ...
            new_lang_text = text_list[lang_start_pos + i].strip()
            if (g_data_json[g_index + i][g_json_key_lang_text] != new_lang_text):
                g_data_json[g_index + i][g_json_key_lang_text] = new_lang_text
                change = True
...
def set_global(load_json, load_list, json_key_text, json_key_path, batch):
    global g_json_key_text, g_json_key_path, g_json_key_lang_text, g_load_file, g_load_format, g_batch
...
if __name__ == "__main__":
    ...
    with gr.Blocks(title="打标WebUI界面") as demo:
            ...
            with gr.Column():
                for _ in range(0,g_batch):
                    with gr.Row():
                        ...
                        g_lang_text_list.append(lang_text)
