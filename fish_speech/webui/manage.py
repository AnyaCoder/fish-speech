...
def check_files(data_path: str, max_depth: int):
    global dict_items
...
with gr.Blocks(head="<style>\n" + css + "\n</style>", js=js,
    ...
    with gr.Row():
            ...
            with gr.Tab("\U0001F4D6 数据集准备"):
                ...
                with gr.Row():
                    transcript_path = gr.Textbox(label="\U0001F4DD 转写文本filelist所在路径",
                                                 ...
                                                 interactive=True)
                    label_radio = gr.Radio(label="(可选)打标语言",
                                           info="如果没有音频对应的文本，则进行辅助打标, 支持.txt或.lab格式",
                                           choices=["中文", "日文", "英文", "不打标"], value="不打标", interactive=True)
                        ...
                        with gr.Accordion("VQGAN配置项", open=False):
                            ...
                            with gr.Accordion("其他配置", open=False):
                                ...
                                vqgan_every_n_steps_slider = gr.Slider(label="每n步保存一个模型", interactive=True,
                                                                       ...
                                                                       value=init_vqgan_yml["callbacks"]["model_checkpoint"]["every_n_train_steps"])
                    with gr.Row():
                        with gr.Accordion("LLAMA配置项", open=False):
                            ...
                            with gr.Accordion("其他配置", open=False):
                                llama_precision_dropdown = gr.Dropdown(label="训练精度", interactive=True,
                                                                       choices=["32", "bf16-true", "16"],
                                                                       value=str(init_llama_yml["trainer"]["precision"]))
                                llama_every_n_steps_slider = gr.Slider(label="每n步保存一个模型", interactive=True,
                                                                       minimum=500, maximum=10000, step=500,
                                                                       value=init_llama_yml["callbacks"]["model_checkpoint"]["every_n_train_steps"])
        ...
        with gr.Column():
            ...
            with gr.Accordion("查看预处理文件夹状态 (滑块为显示深度大小)", elem_classes=["scrollable-component"], elem_id="file_accordion"):
            ...
            with gr.Row(equal_height=False):
                admit_btn = gr.Button("\U00002705 OK", scale=0, min_width=80, variant="primary")
    ...
    admit_btn.click(fn=check_files, inputs=[train_box, tree_slider], outputs=[error, file_markdown])
    ...
    vqgan_trainer_btn.click(fn=modify_vqgan_trainer_param,
                            inputs=[vqgan_val_check_interval_slider, vqgan_maxsteps_slider, vqgan_lr_slider,
                                    vqgan_data_num_workers_slider, vqgan_data_batch_size_slider, vqgan_data_val_batch_size_slider,
    ...
    llama_trainer_btn.click(fn=modify_llama_trainer_param,
                            inputs=[llama_data_max_length_slider, llama_maxsteps_slider, llama_lr_slider,
                                    llama_data_num_workers_slider, llama_data_batch_size_slider, llama_limit_val_batches_slider,
